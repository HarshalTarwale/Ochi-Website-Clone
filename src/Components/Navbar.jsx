...
const Navbar = () => {
  ...
  useEffect(() => {
    // Function to handle scroll behavior - simplified for smoother performance
    const handleScroll = () => {
      const currentScrollY = window.scrollY;

      // Simple logic: show when scrolling up, hide when scrolling down
  ...
  return (
    <div 
      className={`fixed z-[999] w-full px-14 py-4 flex justify-between font-neue text-black transition-transform duration-300 ease-out ${
        isVisible ? 'translate-y-0' : '-translate-y-full'
      }`}
      style={{
        // GLASSMORPHISM EFFECT - CUSTOMIZE HERE:
        backgroundColor: 'rgba(255, 255, 255, 0.1)', // Background transparency (0.0 to 1.0) - lower = more transparent
        backdropFilter: 'blur(10px)', // Blur effect (0px to 20px) - higher = more blur
        WebkitBackdropFilter: 'blur(10px)', // Safari support - keep same as backdropFilter
        borderBottom: '1px solid rgba(255, 255, 255, 0.2)' // Border transparency (0.0 to 1.0)
      }}
    >
      <div className='logo'>
        <svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-black">
      ...
      <div className='links flex gap-8 items-center'>
        {["Services","Our Work","About us","Insights","Contacts us"].map((item,index) => (
          <a key={index} className={`text-md font-light ${index === 4 && "ml-68"}`}>{item}</a>
        ))}
      </div>
    </div>
  )
